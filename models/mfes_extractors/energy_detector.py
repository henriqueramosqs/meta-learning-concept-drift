...
class EnergyDistanceDetector(MfeExtractor):
    
# class EnergyDistanceDetector():
    """
    ...
    def __init__(self, feature_cols: list):
        ...
        self.first_detectors = {
            col: EnergyDistance() for col in feature_cols
        }
        self.last_detectors = {
            col: EnergyDistance() for col in feature_cols
        }
    ...
    def fit(self, data_frame: pd.DataFrame):
        ...
        Returns:
            EnergyDistanceDetector: The fitted instance itself, to allow for method chaining
        """
    ...
    def evaluate(self, data_frame: pd.DataFrame) -> dict:
        ...
        for col in self.feature_cols:
            ...
            results[f"energy_distance_first_{col}_distance"] = distance_result

            drift_detected = distance_result > DRIFT_THRESHOLD
            results[f"energy_distance_first_{col}"] = drift_detected
            if(drift_detected):
                global_drift = True
        
        results["energy_global_first_drift_flag"] = global_drift

        global_drift = False

        for col in self.feature_cols:
            distance_result = self.last_detectors[col].compare(data_frame[col].to_numpy())[0][0]
            results[f"energy_distance_lst_{col}_distance"] = distance_result

            drift_detected = distance_result > DRIFT_THRESHOLD
            results[f"energy_distance_lst_{col}"] = drift_detected
            if(drift_detected):
                global_drift = True

            self.last_detectors[col].fit(data_frame[col].to_numpy())
        
        results["energy_global_lst_drift_flag"] = global_drift
        return results
...
if __name__ == "__main__":
    ...
    detector = EnergyDistanceDetector(feature_cols=['feature'])
