...
class EMDDetector(MfeExtractor):
# class EarthMoversDetector():
    """
    ...
    def __init__(self, feature_cols: list):
        ...
        self.first_detectors = {
            col: EMD() for col in feature_cols
        }
        self.last_detectors = {
            col: EMD() for col in feature_cols
        }
    ...
    def fit(self, data_frame: pd.DataFrame):
        ...
        Returns:
            EarthMoversDetector: The fitted instance (for method chaining).
        """
    ...
    def evaluate(self, data_frame: pd.DataFrame) -> dict:
        ...
        Returns:
            dict: Dictionary containing measured distances, drift/warning flags for each column and a global drift flag.
        """
        results = {}
        global_drift = False
        ...
        for col in self.feature_cols:
            ...
            results[f"emd_first_{col}_distance"] = distance_result

            drift_detected = distance_result > DRIFT_THRESHOLD
            results[f"emd_first_{col}"] = drift_detected
            if(drift_detected):
                global_drift = True
        
        results["emd_global_first_drift_flag"] = global_drift

        global_drift = False

        for col in self.feature_cols:
            distance_result = self.last_detectors[col].compare(data_frame[col].to_numpy())[0][0]
            results[f"emd_lst_{col}_distance"] = distance_result

            drift_detected = distance_result > DRIFT_THRESHOLD
            results[f"emd_lst_{col}"] = drift_detected
            if(drift_detected):
                global_drift = True

            self.last_detectors[col].fit(data_frame[col].to_numpy())
        
        results["emd_global_lst_drift_flag"] = global_drift
        return results
...
if __name__ == "__main__":
    ...
    detector = EMDDetector(feature_cols=['feature'])
