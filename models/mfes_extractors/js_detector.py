...
NUM_BINS = 20
class JensenShanonDetector(MfeExtractor):
    
# class JensenShanonDetector():
    """
    ...
    def __init__(self, feature_cols: list):
        ...
        self.first_detectors = {
            col:   JS(num_bins=20) for col in feature_cols
        }
        self.last_detectors = {
            col:   JS(num_bins=20) for col in feature_cols
        }
    ...
    def fit(self, data_frame: pd.DataFrame):
        ...
        Returns:
            JensenShanonDetector: The fitted instance itself, to allow for method chaining
        """
    ...
    def evaluate(self, data_frame: pd.DataFrame) -> dict:
        ...
        for col in self.feature_cols:
            ...
            results[f"jensen_shanon_distance_first_{col}_distance"] = distance_result

            drift_detected = distance_result > DRIFT_THRESHOLD
            results[f"jensen_shanon_distance_first_{col}"] = drift_detected
            if(drift_detected):
                global_drift = True
        
        results["jensen_shanon_global_first_drift_flag"] = global_drift
        ...
        for col in self.feature_cols:
            ...
            results[f"jensen_shanon_distance_lst_{col}_distance"] = distance_result

            drift_detected = distance_result > DRIFT_THRESHOLD
            results[f"jensen_shanon_distance_lst_{col}"] = drift_detected
            if(drift_detected):
                global_drift = True

            self.last_detectors[col].fit(data_frame[col].to_numpy())
        
        results["jensen_shanon_global_lst_drift_flag"] = global_drift
        return results
...
if __name__ == "__main__":
    ...
    detector = JensenShanonDetector(feature_cols=['feature'])
